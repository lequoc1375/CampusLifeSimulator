...
public interface SubjectRepo extends JpaRepository<Subject, Integer> {
    ...
    @Query("SELECT s FROM Subject s WHERE s.subject_id NOT IN " +
           "(SELECT sr.subject.subject_id FROM SubjectRegister sr WHERE sr.userCurriculum.user.userId = :userId)")
    ...
    @Query("SELECT s FROM Subject s JOIN SubjectRegister sr ON s.subject_id = sr.subject.subject_id " +
           "WHERE sr.userCurriculum.user.userId = :userId AND sr.overall = 'studying'")
    List<Subject> findInProgressSubjectsByUserId(@Param("userId") int userId);

    @Query("SELECT s FROM Subject s JOIN SubjectRegister sr ON s.subject_id = sr.subject.subject_id " +
           "WHERE sr.userCurriculum.user.userId = :userId AND sr.overall IN ('pass', 'fail')")
